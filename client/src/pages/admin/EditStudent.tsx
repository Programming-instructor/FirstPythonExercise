...
const EditStudent = () => {
  ...
  const { data: student, isLoading: isFetching } = useFetchStudentById(id!);
  ...
  useEffect(() => {
    if (student) {
      setFormData({
        ...student,
        guardian: student.guardian || { name: "", relation: "", phone: "" },
        grade: student.grade?.toString() || "",
      });
      if (student.student_portrait_front?.url) {
        setImagePreview(student.student_portrait_front.url); // Assuming URL is absolute or relative correctly
      }
    }
  }, [student]);
  ...
  const handleSubmit = () => {
    ...
    if (imageFile) {
      submitData.append('portrait', imageFile);
    }
    mutate({ id: id!, formData: submitData });
  };
  ...
  return (
    <div className="flex flex-col items-center" dir="rtl">
      <div className="flex flex-col gap-5 bg-neutral-50 px-8 py-12 shadow rounded-2xl w-full max-w-5xl">
        <h1 className="font-bold text-xl m-auto">ویرایش اطلاعات هنرجو</h1>
        {/* Important Notes */}
        ...
        <div className="flex justify-start gap-4 mt-8">
          <Button asChild variant="outline">
            <Link to="/admin/students">بازگشت به لیست</Link>
          </Button>
          <Button disabled={isPending || !isFormComplete()} onClick={handleSubmit}>
            {isPending ? "در حال ویرایش..." : "ویرایش هنرجو"}
          </Button>
        </div>
      </div>
    </div>
