...
import { useEducationalDeputyForm } from "@/hooks/useEducationalDeputyForm";
import { useFetchStudentByCode } from "@/hooks/useFetchStudnetByNationalCode";
import { Link, useOutletContext } from "react-router-dom";
import { useSubmitEducationalDeputyForm } from "@/hooks/useSubmitEducationalDeputyForm";
...
const EducationalDeputy = () => {
  const user: User = useOutletContext();
  const [nationalCode, setNationalCode] = useState("");
  const [submittedCode, setSubmittedCode] = useState("");
  const [message, setMessage] = useState("");
  const { data: student, isLoading, error: studentError } = useFetchStudentByCode(submittedCode);
  const { data: formData, isLoading: formLoading, error: formError } = useEducationalDeputyForm(student?._id);
  const { mutate: submitForm, isPending: isSubmitting } = useSubmitEducationalDeputyForm();
  ...
  return (
    <div className="flex flex-col items-center min-h-screen p-4" dir="rtl">
      <div className="w-full max-w-5xl bg-white shadow rounded-2xl p-8 space-y-8">
        <h1 className="font-bold text-2xl text-center">معاونت آموزشی</h1>
        ...
        {student && (
          formLoading ? (
            ...
            <AccordionForm
              fields={rolesFormFields["معاونت آموزشی"]}
        ...
        {student && (
          <Card className="border-gray-200 shadow-sm">
            <CardContent className="p-6 space-y-6">
              <h2 className="font-semibold text-lg text-gray-800">ثبت گزارش معاونت آموزشی</h2>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="متن گزارش معاونت آموزشی را وارد کنید"
