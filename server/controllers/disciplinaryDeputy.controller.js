...
exports.getFormForStudent = async (req, res) => {
  try {
    ...
    const form = await DisciplinaryDeputy.findOne({ studentId });
    if (!form) {
...
exports.submitForm = async (req, res) => {
  try {
    ...
    // Find existing form
    let form = await DisciplinaryDeputy.findOne({ studentId });

    if (form && form.locked) {
      return res.status(403).json({ message: 'Form is already submitted and locked' });
    }

    formData.deputyDecision = formData.deputyDecision === 'پذیرش';
